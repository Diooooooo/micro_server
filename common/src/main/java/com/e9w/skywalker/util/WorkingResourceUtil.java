...
public class WorkingResourceUtil {
	...
	static {
		...
		try {
			p.load(currentClassLoader().getResourceAsStream("working.properties"));
	...
	public static InputStream getInputStream(String fileName) {
		return currentClassLoader().getResourceAsStream(getWorkingFileName(fileName));
	}

	public static File getFile(String fileName) {
		URL url = currentClassLoader().getResource(getWorkingFileName(fileName));
		return new File(url.getFile());
	}

	public static Map<String, String> loadPropertFileWithWorking(String fileName) {
		Map<String, String> map = new HashMap<String, String>();
		try (InputStream inputStream = currentClassLoader().getResourceAsStream(fileName)){
			Properties p = new Properties();
			p.load(inputStream);
			map.putAll((Map) p);
		} catch (IOException e) {
