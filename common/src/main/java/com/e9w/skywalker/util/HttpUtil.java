package com.e9w.skywalker.util;
...
public class HttpUtil {
	...
	private static ContentType default_content_type = ContentType.create("text/plain", Consts.UTF_8);
	...
	public static String post(String url, Map<String, Object> request)
			throws IOException {
		return post(url, request, Configuration.getCharset(), Configuration.getTimeout());
	}
	...
	private static List<NameValuePair> mapToNameValuePairs(Map<?, ?> request) {
		...
		for (Map.Entry<?, ?> entry : request.entrySet()) {
			...
			if (value != null) {
				NameValuePair nameValuePairs = new BasicNameValuePair(key, value.toString());
	...
	public static String post(String url, String content) throws IOException {
		return post(url, content, default_content_type, Configuration.getCharset(),
	...
	protected static HttpEntity httpEntityRequest(String url,
		...
		try {
			...
			BufferedHttpEntity bufferedHttpEntity = new BufferedHttpEntity(resEntity);
...
//					.setProxy(proxy)
			...
			HttpEntity resEntity = response.getEntity();
            IOUtils.copy(resEntity.getContent(), new FileOutputStream(file));
        } finally {
			close(response);
			close(httpClient);
		}
...
//					.setProxy(proxy)
	...
	public static void main(String[] args) throws IOException {
		...
		// StringCharBuffer scb=new StringCharBuffer();
